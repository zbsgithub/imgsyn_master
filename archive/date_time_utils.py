...
if __name__ == "__main__":
    print
    DateParser().convert("19901", "%Y.%m")

    print
    DateParser().sort_date_dict({'2010-04-01': '122', '2010-04-03': '2342', '2010-10-02': 'wfwe'}, DateType.YMD)

    test_list1 = ['2010-04-01', '2010-04-03', '2010-04-02']
    test_list2 = ['2010-04-02 00:10:00', '2010-04-03 00:00:00', '2010-04-02 00:19:10']
    print
    DateParser().sort_date_list(test_list1, DateType.YMD)
    print
    DateParser().sort_date_list(test_list2, DateType.YMD_HMS)

    print
    DateParser().find_min_date(test_list1, DateType.YMD)
    print
    DateParser().count_time('2010-04-01', DateType.YMD)

    # 返回当前年、月、日
    print
    "年:", datetime.datetime.now().year, "月:", datetime.datetime.now().month, "日:", datetime.datetime.now().day

    # 两个日期的时间差
    delta = datetime.datetime(2011, 1, 7) - datetime.datetime(2008, 6, 24, 8, 15)
    print
    "天:", delta.days, "秒:", delta.seconds

    # 相对时间减法
    print
    "日期相减1天", (datetime.datetime.now() + datetime.timedelta(days=-1)).strftime("%Y-%m-%d %H:%M:%S")
    print
    "日期相减1小时", (datetime.datetime.now() + datetime.timedelta(hours=-1)).strftime("%Y-%m-%d %H:%M:%S")
    print
    "日期相减1分钟", (datetime.datetime.now() + datetime.timedelta(minutes=-1)).strftime("%Y-%m-%d %H:%M:%S")
    print
    "日期相减1秒钟", (datetime.datetime.now() + datetime.timedelta(seconds=-1)).strftime("%Y-%m-%d %H:%M:%S")

    # 日期转字符串
    print
    str(datetime.datetime(2011, 1, 7))
    ...
    parse('201101')
    print
    parse('jan 31, 10:45 PM 1997')
    print
    parse('01/02/2011')
    print
    parse('01/02/2011', dayfirst=True)
    ...
    # 判断字符串是否为指定格式的日期格式
    print
    DateParser().is_valid_date('2010-04-01', DateType.YMD)
